...
GATES: list[dict[str, Any]] = [
    {
        ...
        "does": [
            ...
            "**G0-R2 確認提升**:不可信來源(tool_output / user_unverified)若有一個**獨立的**"
            "已驗證用戶或系統節點帶 `confirms` 且 `confirmed_action_hash` 等於本次動作由 runtime"
            "算出的指紋,授權上限提升為確認者的上限。指紋涵蓋全部參數,由 harness 計算、"
            "不接受 LLM 自報;沒有指紋一律不提升(fail-closed);memory 不可被提升。",
        ],
        "decision": {
            "title": "取最弱環節,不取最後一手;但確認過的動作可以被提升",
            "body": "已驗證用戶把 PDF 的內容轉述一次,不會把 untrusted 洗成 verified —— "
                    "轉述只是多一個指令來源節點,沒有 confirms、沒有指紋。"
                    "確認則是 harness 把「動作 + 全部參數」呈現給用戶、用戶同意的獨立事件。"
                    "少了 G0-R2,「用戶上傳帳單截圖後說就退這 880 元」會整組被擋;"
                    "有了它,PDF 夾帶的「匯出全部客戶」仍在 G0 被攔,因為沒有人確認過那個動作。",
        },
