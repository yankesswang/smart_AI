...
| Item | Value |
| --- | --- |
| Project Name | smart_factory_core_20260906 |
...
| AI Image Acquisition Path | api — 沿用 smart_factory_20260904 已生成之圖片，本專案不再呼叫生圖 |
...
- **Template Application**: 套用 presentation_core 版型系統（Layout，`template_reuse_scope: layout`、`template_adherence: adaptive`）：15 頁全部從既有原型出發，Master 沿用 presentation_core，Layout 定義依本簡報的內容形狀在 lock 中重新宣告（複合內容改以 proxy 綁定、資料頁的欄寬對齊圖表面板）—— P01 `01_title_slide`、P02 `13_kpi_dashboard`、P03 與 P06 `12_three_card`、P04 `02_title_content`、P05 `17_two_picture_caption`、P07 與 P09 `05_comparison`、P08 `08_content_caption`、P10 `20_table_summary`、P11–P13 `19_chart_insight`、P14 `15_data_story`、P15 `04_two_content`。版型的 Master 底、版面框與 slot 幾何一律保留；框與線的顏色重上為本專案色盤（工業藍 #0F4C81 為系統色、hazard red #E2231A 只給工安與否決、驗證綠 #1B7F5A 只給通過）；slot 字級改用本專案 typography 角色（title 42 / subtitle 32 / lead 30 / body 24 / annotation 18 / footnote 16 / data 22）；複合內容（KPI 卡、八階段軌道、方案排名理由）放進 object slot 並以 proxy 綁定；圖表與表格直接作為 chart / table slot 的 carrier。原版（smart_factory_20260904）的內容、數字與講稿逐頁沿用；原版 P05 的四張截圖在版型版只保留機台遙測與閉環進度兩張，P11 的示意圖不放。
...
- **Image Rendering Behavior**: 扁平向量插畫，等距或正視，白底無場景雜訊；只用本專案色盤（工業藍 #0F4C81 承載機構與結構、灰階承載次要體積、hazard red #E2231A 只給警示光暈與危險區、黃黑斜紋只給地面警示帶）；不畫文字、不畫商標、不用照片級材質與陰影堆疊，讓插圖與版型的白底面板同屬一個平面。
- **Visual**: 工廠現場的最小可辨識物件——加工機、輸送帶、包裝機、危險區警示帶、監視攝影機、5G 小基站與邊緣機櫃；人物一律為無臉剪影，只表達站位與是否在危險區內。
- **Mood**: 冷靜的工程說明圖，不是行銷視覺；插圖的職責是讓評審在半秒內認出「這是產線與危險區」，隨即把注意力交還給旁邊的數字。
- **Provenance**: 全部沿用 smart_factory_20260904 已生成的圖片（OpenAI gpt-image-1，2026-09-06），本專案不再呼叫生圖；畫面上一律標示為示意圖。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| fg_cover_hero.png | 1024×1536 | 0.67 | P01 封面右下：產線、紅色警示的加工機、危險區與站在區外的人員 | Illustration | 直立右欄置於副標之下，不與 title / subtitle slot 重疊 | no-crop | ai | Generated | 沿用 smart_factory_20260904（OpenAI gpt-image-1，2026-09-06） | none | local |
| guardian_hardware_state.png | 2104×930 | 2.26 | P05 左圖：主要加工機 VIB 8.89 mm/s CRITICAL、健康度 52、TTT 已踩線 | Screenshot | 左 picture slot，meet 不裁切 | no-crop | user | Existing | 由執行中的服務擷取（2026-09-06） | none | local |
| guardian_agent_loop.png | 1972×608 | 3.24 | P05 右圖：閉環停在第 5 格安全把關與第 6 格核准 | Screenshot | 右 picture slot，meet 不裁切 | no-crop | user | Existing | 由執行中的服務擷取（2026-09-06） | none | local |
| fg_landing_5g.png | 1536×1024 | 1.50 | P15 右欄：工廠 → 5G 小基站 → 邊緣機櫃 → 雲 | Illustration | 右 object slot 內的插圖，與通路句並置 | no-crop | ai | Generated | 沿用 smart_factory_20260904（OpenAI gpt-image-1，2026-09-06） | none | local |
