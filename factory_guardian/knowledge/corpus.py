...
MANUALS: tuple[ManualDoc, ...] = (
    ManualDoc(
        ...
        body=(
            ...
            "⚠ 重要：上述門檻為同型號設備的通用值，僅供快速篩檢。"
            "劣化判讀必須以**該機台自己的交機驗收基準**（COMM 系列文件）為準，"
            "而非本節的通用門檻。同型號機台因組裝公差、地基剛性與安裝水平度不同，"
            "振動基準可能相差 20% 以上（例如 M-A 為 1.9 mm/s、M-B 為 2.6 mm/s）；"
            "同樣讀到 3.6 mm/s，對前者是明顯劣化，對後者仍在正常散布範圍內。"
            "直接套用通用門檻會同時造成漏檢與誤報。"
        ),
    ),
    ManualDoc(
        ...
        body=(
            ...
            "徵兆區間（相對該機台交機驗收基準的變化量，非絕對值）："
            "Vibration +1.8~+9.2 mm/s；Temperature +4~+21°C；Current +0.3~+3.1 A；RPM -6.5~+0.5%。"
            "上述為原廠試驗機的經驗範圍，各廠實際機台會有 15~30% 差異，"
            "判讀時務必以該機台自己的交機基準（COMM 文件）為準。\n"
            "溫升速率：每 10 分鐘 +0.8~+6.5°C，屬緩升型，典型發展以小時計。\n"
            "關鍵鑑別點：Vibration 的上升幅度顯著大於 Temperature 的上升幅度；"
            "量化判準為 ΔVibration/ΔTemperature 介於 0.28~1.60。"
            "若該比值低於 0.10（溫度大幅上升而 Vibration 幾乎不動），"
            "應優先考慮冷卻系統失效而非軸承。\n"
            "次要鑑別點：ΔRPM/ΔCurrent 介於 -3.2~-0.4，即電流小幅上升而轉速僅略降；"
            "若轉速大幅掉落（比值低於 -6），應優先考慮馬達過載。\n"
    ...
    ManualDoc(
        ...
        body=(
            ...
            "徵兆區間（相對該機台交機驗收基準的變化量）："
            "Temperature +9~+33°C；Vibration -0.3~+2.2 mm/s；Current -0.5~+1.4 A；RPM -3.1~+0.8%。"
            "注意 Vibration 區間上限並非 0 —— 冷卻失效時主軸熱變形仍會造成輕微振動上升，"
            "「振動完全不動」不是必要條件。\n"
            "溫升速率：每 10 分鐘 +7~+26°C，屬急升型。上升速率本身就是與軸承劣化的鑑別點。\n"
            "關鍵鑑別點：溫度單獨異常。量化判準為 ΔVibration/ΔTemperature 介於 -0.05~0.14；"
            "若該比值超過 0.28，則不是單純冷卻問題，應考慮軸承劣化。\n"
            "次要鑑別點：ΔCurrent/ΔTemperature 介於 -0.03~0.09，"
            "即電流不隨溫度上升 —— 代表機械負載未增加，熱源來自冷卻不足而非做功。\n"
    ...
    ManualDoc(
        ...
        body=(
            ...
            "徵兆區間（相對該機台交機驗收基準的變化量）："
            "Current +1.2~+8.0 A；RPM -24~-3%；Temperature +2~+18°C；Vibration -0.3~+2.6 mm/s。"
            "注意 Temperature 區間與軸承劣化大幅重疊，不可單憑溫度判別。\n"
            "溫升速率：每 10 分鐘 +2~+11°C。溫升是過載的後果而非主因，"
            "電流與轉速幾乎同時變化（電氣響應快），這是時序上的鑑別點。\n"
            "關鍵鑑別點：電流上升與轉速下降同時發生，是與軸承劣化最主要的區別；"
            "量化判準為 ΔRPM/ΔCurrent 介於 -6.0~-1.1。"
            "軸承劣化的 RPM 通常仍維持在 95% 附近，該比值不會低於 -3.2。\n"
            "次要鑑別點：ΔVibration/ΔCurrent 介於 -0.10~0.55，"
            "即振動相對電流的上升幅度有限，機械性劣化不是主因。\n"
